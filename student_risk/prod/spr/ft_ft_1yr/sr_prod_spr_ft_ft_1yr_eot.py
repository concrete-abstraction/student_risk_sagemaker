...
if now_year < eot_year:
	...
	if snap_check != 3:
		raise config.EOTError(f'{date.today()}: No eot date exception but snapshot exception, attempting to run from prior miderm.')

	else:
		print(f'{date.today()}: No eot date or snapshot exceptions, running from eot.')
